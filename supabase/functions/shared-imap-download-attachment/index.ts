...
import {
  corsHeaders, errorResponse, getAdminUser, loadSharedAccount, getServiceClient,
...
async function streamToBuffer(stream: any): Promise<Uint8Array> {
  ...
  for await (const chunk of stream) chunks.push(chunk instanceof Uint8Array ? chunk : new Uint8Array(chunk));
  let total = 0;
...
Deno.serve(async (req) => {
  ...
  const { data: cached } = await svc
    .from("shared_email_messages_cache").select("attachments")
    .eq("account_id", accountId).eq("folder", folder).eq("uid", Number(uid)).maybeSingle();

  const idx = Number(attachmentId);
  const attMeta = (cached?.attachments as any[] | null)?.[idx];
  if (!attMeta || !attMeta.path) {
    return errorResponse("Attachment-Metadaten nicht gefunden — bitte E-Mail erneut öffnen", 404);
  }
  ...
  try {
    const buffer = await withAccountLock(accountId, () =>
      ...
      })(), 45_000, "shared-imap-download-attachment"),
    );
