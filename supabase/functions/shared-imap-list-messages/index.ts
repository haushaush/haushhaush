...
Deno.serve(async (req) => {
  ...
  console.log(`[shared-imap-list-messages] start accountId=${accountId} folder=${folder}`);
  ...
  try {
    const messages = await withAccountLock(accountId, () =>
      withTimeout((async () => {
        ...
        try {
          ...
          if (search) {
            if (typeof search === "string") {
              ...
              if (token === "UNSEEN" || token === "UNREAD") searchQuery = { seen: false };
              else if (token === "FLAGGED" || token === "STARRED") searchQuery = { flagged: true };
              else searchQuery = { or: [{ subject: search }, { from: search }, { body: search }] };
            } else if (typeof search === "object") {
              const parts: any = {};
              if (search.unseen) parts.seen = false;
              if (search.flagged) parts.flagged = true;
              if (search.query) {
                searchQuery = { and: [parts, { or: [{ subject: search.query }, { from: search.query }, { body: search.query }] }] };
              } else {
        ...
        return collected;
      })(), 60_000, "shared-imap-list-messages"),
    );
    ...
    if (messages.length > 0) {
      const rows = messages.map((m) => ({
        account_id: accountId, folder, uid: m.uid,
        message_id: m.messageId, from_address: m.from_address, from_name: m.from_name,
        to_addresses: m.to_addresses, cc_addresses: m.cc_addresses,
        subject: m.subject, snippet: null, date: m.date,
        flags: m.flags, has_attachment: m.has_attachment, size_bytes: m.size_bytes,
        fetched_at: new Date().toISOString(),
      }));
      await svc.from("shared_email_messages_cache").upsert(rows, {
        onConflict: "account_id,folder,uid", ignoreDuplicates: false,
      });
    }

    await svc.from("shared_email_accounts")
      .update({ last_polled_at: new Date().toISOString() })
      .eq("id", accountId);

    let filterUnseen = false, filterFlagged = false, textQuery: string | null = null;
    if (search) {
      if (typeof search === "string") {
        const token = search.trim().toUpperCase();
    ...
    if (filterFlagged) cacheBuilder = cacheBuilder.contains("flags", ["\\Flagged"]);
    if (textQuery) {
    ...
    if (filterUnseen) cached = cached.filter((m: any) => !(m.flags ?? []).includes("\\Seen"));

    console.log(`[shared-imap-list-messages] done in ${Date.now() - start}ms (${messages.length} new, ${cached.length} returned)`);
    return jsonResponse({ ok: true, messages: cached });
  } catch (err) {
    console.error(`[shared-imap-list-messages] error:`, (err as Error).message);
...
function hasAttachmentInStructure(bs: any): boolean {
  if (!bs) return false;
  if (Array.isArray(bs.childNodes)) return bs.childNodes.some((c: any) => hasAttachmentInStructure(c));
