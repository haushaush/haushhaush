...
Deno.serve(async (req) => {
  ...
  // Only poll accounts where user was active in the last 24h
  const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const { data: accounts, error } = await svc
    .from("email_accounts")
    .select("id, imap_host, imap_port, imap_secure, imap_user, imap_password_encrypted, last_user_activity_at")
    .eq("is_active", true)
    .gte("last_user_activity_at", cutoff);
  ...
  for (const acc of accounts) {
    try {
      const { data: pwData, error: pwErr } = await svc.rpc("decrypt_imap_password", {
        encrypted: acc.imap_password_encrypted,
        encryption_key: encryptionKey,
      });
      if (pwErr || !pwData) continue;

      // Find max known uid for INBOX
      ...
      try {
        ...
        try {
          ...
          if (rows.length > 0) {
            await svc.from("email_messages_cache").upsert(rows, { onConflict: "account_id,folder,uid" });
      ...
      await svc.from("email_accounts").update({ last_polled_at: new Date().toISOString() }).eq("id", acc.id);
      polled++;
    } catch {
