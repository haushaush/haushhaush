  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-3"><Skeleton className="h-10 w-64" /><Skeleton className="h-10 w-40" /></div>
    ...
    <div className="space-y-6">
            ...
            <DialogContent className="max-w-lg">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><Label>Zahlstatus</Label><Input value={form.zahlstatus || ''} onChange={e => setForm({ ...form, zahlstatus: e.target.value })} /></div>
                  <div><Label>Laufzeit</Label><Input value={form.laufzeit || ''} onChange={e => setForm({ ...form, laufzeit: e.target.value })} /></div>
                </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-[150px]"><SelectValue placeholder="Kundenstatus" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filterAmpel} onValueChange={setFilterAmpel}>
          <SelectTrigger className="w-[140px]"><SelectValue placeholder="Ampel" /></SelectTrigger>
          <SelectContent>
        ...
        {branchen.length > 0 && (
          <Select value={filterBranche} onValueChange={setFilterBranche}>
            <SelectTrigger className="w-[140px]"><SelectValue placeholder="Branche" /></SelectTrigger>
            <SelectContent>
        ...
        {projekttypen.length > 0 && (
          <Select value={filterProjekttyp} onValueChange={setFilterProjekttyp}>
            <SelectTrigger className="w-[140px]"><SelectValue placeholder="Projekttyp" /></SelectTrigger>
            <SelectContent>
          ...
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                <TableRow key={c.id} className="cursor-pointer hover:bg-primary/5" onClick={() => navigate(`/kunden/${c.id}`)}>
                  ...
                  <TableCell>
                    <span className="flex items-center gap-2">
                      <span className={`h-2.5 w-2.5 rounded-full ${AMPEL_DOT[c.ampelstatus] || 'bg-muted'}`} />
                      <span className="text-xs text-muted-foreground">{c.ampelstatus}</span>
                    </span>
                  </TableCell>
                  <TableCell className="text-muted-foreground">{c.projekttyp || '–'}</TableCell>
                  <TableCell className="text-muted-foreground">{c.startdatum || '–'}</TableCell>
                  <TableCell className="text-muted-foreground">{c.laufzeit || '–'}</TableCell>
