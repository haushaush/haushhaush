      ...
      <select
        ...
        className={`
          ...
          ${isActive
            ...
            : 'bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-sm'
          }
        `}
      >
...
function PrimaryHighlight({ item }: { item: AnyItem }) {
  ...
  if (item._type === 'werbeanzeige') {
    ...
    return (
      <p className="text-sm font-semibold text-purple-600 dark:text-purple-400 mb-3 inline-flex items-center gap-1.5">
        {isVideo ? <Video className="w-3.5 h-3.5" /> : <ImageIcon className="w-3.5 h-3.5" />}
        {item.creative_format || item.ad_format || (isVideo ? 'Video' : 'Bild')}
      </p>
    );
  }
  return null;
