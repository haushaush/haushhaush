  ...
  const filtered = useMemo(() => {
    ...
    return accounts.filter(
      (acc) =>
        ...
        String(acc.account_id ?? acc.id ?? "").toLowerCase().includes(q),
    );
  }, [accounts, search]);
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 focus:border-gray-400 dark:focus:border-gray-500 outline-none"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm text-gray-500 dark:text-gray-400">Kein Werbekonto gefunden.</p>
          </div>
        ) : (
